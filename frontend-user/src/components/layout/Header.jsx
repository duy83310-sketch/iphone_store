    ...
    <header className="header">
      <div className="header-top">
        ...
        <div className="header-actions">
          {user ? (
            <div className="avatar-wrap" title={user.name || "Tài khoản"}>
              {user.avatar ? (
                <img src={user.avatar} alt={user.name || "avatar"} className="avatar-img" />
              ) : (
                <div className="avatar-placeholder">{(user.name || "U").slice(0,1).toUpperCase()}</div>
              )}

              <div className="avatar-dropdown">
                <div style={{ padding: 8, color: "#fff", fontWeight: 600 }}>{user.name}</div>
                <button onClick={logout} className="logout-btn small">Đăng xuất</button>
              </div>
            </div>
          ) : (
